...
def main():
    ...
    print(f"Total cells: {len(cell_dict)}") # ap
    ...
    cluster_predefine = get_predefine_cluster(edge_index=edge_index,
    ...
    if args.mode == 'train':
        if args.pretrain and args.weight_path != '':
            model.GNN_drug.load_state_dict(
                torch.load('./model_pretrain/{}.pth'.format(
                    args.weight_path))['model_state_dict'])
        ...
        test_rmse, test_MAE, test_r2, test_r = validate(model, test_loader, args.device)
        print('Test RMSE: {}, MAE: {}, R2: {}, R: {}'.format(
    ...
    te_true, te_pred = predict(model, test_loader, args.device)
    te_true = te_true.cpu().numpy().flatten()
    te_pred = te_pred.cpu().numpy().flatten()
    ...
    # ccp_scr = pearsonr(te_true, te_pred)[0]
