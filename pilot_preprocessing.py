...
if os.path.exists(cell_dict_save_to) and os.path.exists(edge_index_fn) and os.path.exists(selected_gene_fn):
    ...
    mu_df = pd.read_csv(improve_globals.gene_mutation_file_path, sep="\t", index_col=0, skiprows=[0, 2], low_memory=False)
    # mu_df = pd.read_parquet(improve_globals.gene_mutation_file_path)
    # mu_df.set_index("Gene_symbol", inplace=True)
    # mu_df_tmp = mu_df.drop(mu_df.iloc[:, 1:14], axis=1).T
    # a = mu_df_tmp.duplicated()
    # list(a)

    # #
    # mu_df_tmp.columns = mu_df_tmp.iloc[0]
    # mu_df = mu_df_tmp.rename(columns=mu_df_tmp.iloc[1])
    # mu_df = mu_df_tmp.rename(columns=mu_df_tmp.iloc[0]).loc[1:]
    ...
    cell_exp.fillna(0)  #  impute missing values
    cell_cn_df = cn_df[selected_gene]
    cell_cn = cell_cn_df.iloc[1:]
    cell_cn.fillna(0)  #  impute missing values

    # mu_df_tmp.set_index("Gene_symbol", inplace=True)
    cell_mu_df = mu_df[selected_gene]
    # cell_mu_df = mu_df.loc[gene_list]
    # cell_mu = cell_mu_df.iloc[13:].T #TODO: 11?
    # cell_mu.tail(cell_mu.shape[0] - 13)
    ...
    # cell_mu_index = set(cell_mu.index)
    # selected_cell_lines = cell_mu_index.intersection(cell_exp_index.intersection(cell_cn_index))
    # cell_line_ids = list(selected_cell_lines)
    # set(cell_line_ids)
    ...
    for cl_df in enumerate(cell_line_ids):
        ...
        mu = list(cell_mu.loc[cl])  # TODO: cell_mu.iloc()?
        # mu = list(cell_mu.iloc[cl])
        exp = convert_float(exp)
        cn = convert_float(cn)
        mu = convert_float(mu)
        cell_dict[cl] = Data(x=torch.tensor(np.array([exp, cn, mu], dtype=float).T, dtype=torch.float))
